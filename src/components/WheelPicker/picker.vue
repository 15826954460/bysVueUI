...
export default {
  ...
  data() {
    return {
      ...
      groupsRectList: new Array(this.arrList.length), // 保存元素相关位置信息列表
      dragInfo: {
  ...
  created() {
    this.getInitialCurrentIndexList();
  },
  mounted() {
  ...
  methods: {
    ...
    setGroupData(gIndex, groupData) {
      ...
      this.$set(this.arrList, gIndex, groupData);
    },
    // 对外暴露的方法
    getGroupsRectList() {
    ...
    getInitialCurrentIndexList() {
      let _currentIndexList = this.arrList.map((item) => {
      ...
      this.currentIndexList = _currentIndexList;
    },
    // 创建dom变化监听
    _createDomObserver() {
    ...
    _correctionCurrentIndex(ev, gIndex) {
      let timer = setTimeout(() => {
        if (
          ...
          if (unsafeGroupIndex > this.arrList[gIndex].list.length - 1) {
            movedIndex = this.arrList[gIndex].list.length - 1;
